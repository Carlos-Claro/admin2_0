...
{       
	...
	private $valida = array(
                                ...
                                array( 'field'   => 'endereco',           'label'   => 'Endereco', 	'rules'   => 'trim'),
                                array( 'field'   => 'meta_titulo',            'label'   => 'Meta Titulo',         'rules'   => 'trim'),
                                array( 'field'   => 'meta_descricao',           'label'   => 'Meta descricao', 	'rules'   => 'trim'),
                                array( 'field'   => 'meta_keys',           'label'   => 'Meta Keys', 	'rules'   => 'trim'),
                                );
	...
	public function listar($coluna = 'nome', $ordem = 'ASC', $off_set = 0)
	{
            ...
            $itens = $this->categorias_model->get_itens( $filtro->get_filtro(), $coluna, $ordem, $off_set );
            $total = $this->categorias_model->get_total_itens( $filtro->get_filtro() );
            ...
            $this->layout
                        ...
                        ->set_include('js/categorias.js', TRUE)
                        ->set_include('css/estilo.css', TRUE)
                        ->set_breadscrumbs('Painel', 'painel',0)
                        ->set_breadscrumbs('Categorias', 'categorias', 1)
                        ->set_usuario()
	...
	public function exportar()
	{
		...
		$valores = ( isset($_GET['b']) ? $_GET['b'] : array() );
		$filtro = $this->_inicia_filtros( $url, $valores );
		$data = $this->categorias_model->get_itens( $filtro->get_filtro() );
	...
	public function adicionar()
	{
		...
		{
			$data = $this->_post();
                        $id = $this->categorias_model->adicionar($data);
		...
		else
		{
			...
			$data['tipo'] = 'Categorias Adicionar';	
			$this->layout
				->set_function( $function )
				->set_include('js/categorias.js', TRUE)
				...
				->set_include('css/estilo.css', TRUE)  
                                ...
                                ->set_breadscrumbs('Adicionar', 'categorias/adicionar', 1)
				->set_usuario($this->set_usuario())
                                ->set_menu($this->get_menu($class, $function))
				->view('add_categorias',$data);
		}   
	...
	{
		$dados = $this->categorias_model->get_item($codigo);
		if ($dados)
		{
			$this->form_validation->set_rules($this->valida);
			if ($this->form_validation->run())
			{
                            $data = $this->_post();
                            $id = $this->categorias_model->editar($data, array('categorias.id' => $codigo));
			...
			else
			{
				...
				$data['tipo'] = 'Categorias Editar';//$data = $this->_init_selects();
				$data['item'] = $dados;
				$data['mostra_id'] = TRUE;
				...
				$this->layout
					...
					->set_include('css/estilo.css', TRUE)
                                        ->set_breadscrumbs('Painel', 'painel',0)
                                        ->set_breadscrumbs('Categorias', 'categorias', 0)
                                        ->set_breadscrumbs($dados->titulo, 'categorias', 1)
                                        //->set_breadscrumbs('Editar', 'categorias/editar/'.$codigo, 1)
					->set_usuario($this->set_usuario())
                                        ->set_menu($this->get_menu($class, $function))
					->view('add_categorias',$data);
			}
        ...
        /**
         * Chama a classe monta_link e mostra esse link
         * @version 1.0
         * @access public
         */
	...
	public function remover($id = NULL)
	{
		...
		$quantidade = $this->categorias_model->excluir('categorias.id in ('.implode(',',$selecionados).')');
		if ($quantidade>0)
		{
	...
	private function _inicia_listagem( $itens, $extras = NULL, $exportar = FALSE )
	{
		...
		else 
		{
			$data['cabecalho'] = array(
                                                    ...
                                                    (object)array( 'chave' => 'titulo', 'titulo' => 'Nome', 	'link' => str_replace(array('[col]','[ordem]'), array('titulo',( ($extras['ordem'] == 'ASC' && $extras['col'] == 'titulo') ? 'DESC' : 'ASC' ) ), $extras['url']), 'class' => ( ($extras['col'] == 'titulo' ) ? 'ui-state-highlight'.( ($extras['col'] == 'titulo' && $extras['ordem'] == 'ASC') ? ' ui-icon-caract-1-n' : ' ui-icon-caract-1-s' ) : 'ui-state-disabled ui-icon-caract-1-s' ), 'classe_destino' => 'subcategorias/listar/[id]' ),
	...
	{
                $config['itens'] = array(
                                        array( 'name' => 'id',              'titulo' => 'ID: ',             'tipo' => 'text', 'valor' => '', 'classe' => 'ui-state-default', 'where' => array( 'tipo' => 'where', 	'campo' => 'categorias.id', 	'valor' => '' ) ),
                                        array( 'name' => 'nome',          'titulo' => 'Nome: ',           'tipo' => 'select', 'valor' => $this->categorias_model->get_select(), 'classe' => 'form-control ui-state-default', 'where' => array( 'tipo' => 'where', 	'campo' => 'categorias.id', 	'valor' => '' ) ),
                                        array( 'name' => 'link',           'titulo' => 'Link: ',         'tipo' => 'text', 'valor' => '', 'classe' => 'ui-state-default', 'where' => array( 'tipo' => 'like', 	'campo' => 'categorias.link', 		'valor' => '' ) ),
                                        );	
 		$config['colunas'] = 3;
 		$config['extras'] = '';
 		$config['url'] = $url;
