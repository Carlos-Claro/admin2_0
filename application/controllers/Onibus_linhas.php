...
/**
 * Página de gerenciamento de linhas de onibus
 * @version 1.0
 * @access public
 * @package onibus_linhas
 */
class Onibus_Linhas extends MY_Controller 
{       
        /**
	...
	private $valida = array(
                                array( 'field'   => 'id_categoria',           'label'   => 'Categoria', 		'rules'   => 'required'),
                                array( 'field'   => 'nome',           'label'   => 'Nome', 		'rules'   => 'required|trim'),
                                array( 'field'   => 'texto',            'label'   => 'Texto', 		'rules'   => 'trim'),
                                array( 'field'   => 'itinerario',           'label'   => 'Itinerario', 	'rules'   => 'trim'),
	...
	{
            parent::__construct();
            $this->load->model('onibus_linhas_model');
	}
	...
	public function index()
	{
        ...
        /**
         * cria a listagem de onibus_linhas carregando inicia filtros, itens, total itens,
         * inicia listagem, definir a URL da página, chama o onibus_linhas_model que vai 
	...
	public function listar($coluna = 'id', $ordem = 'DESC', $off_set = 0)
	{
            ...
            $itens = $this->onibus_linhas_model->get_itens( $filtro->get_filtro(), $coluna, $ordem, $off_set );
            $total = $this->onibus_linhas_model->get_total_itens( $filtro->get_filtro() );
            ...
            $this->layout
                        ...
                        ->set_include('js/onibus_linhas.js', TRUE)
                        ->set_include('css/estilo.css', TRUE)
                        ->set_breadscrumbs('Painel', 'painel',0)
                        ->set_breadscrumbs('Onibus Linhas', 'onibus_linhas', 1)
                        ->set_usuario()
        ...
        /**
         * exportar uma lista linhas de onibus para um arquivo excel
         * @version 1.0
         * @access public
         */
	public function exportar()
	{
		...
		$valores = ( isset($_GET['b']) ? $_GET['b'] : array() );
		$filtro = $this->_inicia_filtros( $url, $valores );
		$data = $this->onibus_linhas_model->get_itens( $filtro->get_filtro() );
	...
	public function adicionar()
	{
		...
		{
			$data = $this->_post();
			$id = $this->onibus_linhas_model->adicionar($data);
		...
		else
		{
			...
			$class = strtolower(__CLASS__);
                        ...
                        $data['ckeditor_itinerario'] = $this->inicia_ckeditor('itinerario');
			$data['action'] = base_url().$class.'/'.$function;
			$data['tipo'] = 'Onibus Linha Adicionar';	
			$this->layout
				->set_function( $function )
				->set_include('js/onibus_linhas.js', TRUE)
				->set_include('css/estilo.css', TRUE)  
                                ->set_breadscrumbs('Painel', 'painel',0)
                                ->set_breadscrumbs('Onibus Linhas', 'onibus_linhas', 0)
                                ->set_breadscrumbs('Adicionar', 'onibus_linhas/adicionar', 1)
				->set_usuario($this->set_usuario())
                                ->set_menu($this->get_menu($class, $function))
				->view('add_onibus_linhas',$data);
		}   
	...
	{
		$dados = $this->onibus_linhas_model->get_item($codigo);
		if ($dados)
		{
			$this->form_validation->set_rules($this->valida);
			if ($this->form_validation->run())
			{
                                $data = $this->_post();
                                $id = $this->onibus_linhas_model->editar($data, 'onibus_linhas.id = '.$codigo);
			...
			else
			{
				...
				$data['tipo'] = 'Onibus Linhas Editar';//$data = $this->_init_selects();
				$data['item'] = $dados;
				$data['mostra_id'] = TRUE;
				...
				$this->layout
					...
					->set_include('css/estilo.css', TRUE)
                                        ->set_breadscrumbs('Painel', 'painel',0)
                                        ->set_breadscrumbs('Onibus Linhas', 'onibus_linhas', 0)
                                        ->set_breadscrumbs($dados->nome, 'onibus_linhas/editar/'.$codigo, 1)
                                        //->set_breadscrumbs('Editar', 'onibus_linhas/editar/'.$codigo, 1)
					->set_usuario($this->set_usuario())
                                        ->set_menu($this->get_menu($class, $function))
					->view('add_onibus_linhas',$data);
			}
        ...
        {
            $retorno['categorias'] = $this->onibus_linhas_model->get_select_categoria();
            return $retorno;
        }
        
        /**
         * 
         * Deleta um onibus_linhas e suas conexões
         * @param string $id
         * @version 1.0
         * @access public
         */
	public function remover($id = NULL)
	{
		...
		$quantidade = $this->onibus_linhas_model->excluir('onibus_linhas.id in ('.implode(',',$selecionados).')');
		if ($quantidade>0)
		{
        ...
        /**
         * Cria uma lista de linhas de onibus no estilo listagem normal,
	...
	private function _inicia_listagem( $itens, $extras = NULL, $exportar = FALSE )
	{
        ...
        /**
         * Cria um filtro por id_categoria, nome e link
	...
	{
                $config['itens'] = array(
                                        //array( 'name' => 'id',              'titulo' => 'ID: ',             'tipo' => 'text', 'valor' => '', 'classe' => 'ui-state-default', 'where' => array( 'tipo' => 'where', 	'campo' => 'onibus_linhas.id', 	'valor' => '' ) ),
                                        array( 'name' => 'id_categoria',          'titulo' => 'Categoria: ',           'tipo' => 'select', 'valor' => $this->onibus_linhas_model->get_select_categoria(), 'classe' => 'form-control ui-state-default', 'where' => array( 'tipo' => 'like', 	'campo' => 'onibus_linhas.id_categoria', 	'valor' => '' ) ),
                                        array( 'name' => 'nome',          'titulo' => 'Titulo: ',           'tipo' => 'select', 'valor' => $this->onibus_linhas_model->get_select(), 'classe' => 'form-control ui-state-default', 'where' => array( 'tipo' => 'like', 	'campo' => 'onibus_linhas.id', 	'valor' => '' ) ),
                                        array( 'name' => 'link',           'titulo' => 'Link: ',         'tipo' => 'text', 'valor' => '', 'classe' => 'ui-state-default', 'where' => array( 'tipo' => 'like', 	'campo' => 'onibus_linhas.link', 		'valor' => '' ) ),
                                        );	
 		$config['colunas'] = 3;
 		$config['extras'] = '';
 		$config['url'] = $url;
