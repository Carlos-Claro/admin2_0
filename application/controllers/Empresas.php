...
class Empresas extends MY_Controller 
{
	...
	private function _inicia_listagem_administrar( $itens, $extras = NULL, $exportar = FALSE )
	{
		...
		else 
		{
			$data['cabecalho'] = array(
                                                    ...
                                                    (object)array( 'chave' => 'subcategoria',   'titulo' => 'Categoria',    'link' => str_replace(array('[col]','[ordem]'), array('subcategoria',( ($extras['ordem'] == 'ASC' && $extras['col'] == 'subcategoria') ? 'DESC' : 'ASC' ) ), $extras['url']), 'class' => ( ($extras['col'] == 'subcategoria' ) ? 'ui-state-highlight'.( ($extras['col'] == 'subcategoria' && $extras['ordem'] == 'ASC') ? ' ui-icon-caract-1-n' : ' ui-icon-caract-1-s' ) : 'ui-state-disabled ui-icon-caract-1-s' ) ),
                                                    (object)array( 'chave' => 'nome_fantasia',  'titulo' => 'Nome fantasia','link' => str_replace(array('[col]','[ordem]'), array('nome_fantasia',( ($extras['ordem'] == 'ASC' && $extras['col'] == 'nome_fantasia') ? 'DESC' : 'ASC' ) ), $extras['url']), 'class' => ( ($extras['col'] == 'nome_fantasia' ) ? 'ui-state-highlight'.( ($extras['col'] == 'nome_fantasia' && $extras['ordem'] == 'ASC') ? ' ui-icon-caract-1-n' : ' ui-icon-caract-1-s' ) : 'ui-state-disabled ui-icon-caract-1-s' ) ),
                                                    (object)array( 'chave' => 'ultima_integracao',  'titulo' => 'Ultima integração','link' => str_replace(array('[col]','[ordem]'), array('ultima_integracao',( ($extras['ordem'] == 'ASC' && $extras['col'] == 'ultima_integracao') ? 'DESC' : 'ASC' ) ), $extras['url']), 'class' => ( ($extras['col'] == 'ultima_integracao' ) ? 'ui-state-highlight'.( ($extras['col'] == 'ultima_integracao' && $extras['ordem'] == 'ASC') ? ' ui-icon-caract-1-n' : ' ui-icon-caract-1-s' ) : 'ui-state-disabled ui-icon-caract-1-s' ) ),
                        ...
                        $data['operacoes'] = array(
                                                    ...
                                                    (object) array('titulo' => 'Integrar agora', 'class' => 'btn btn-warning', 'icone' => '<span class="glyphicon glyphicon-pencil"></span>', 'link' => 'integracao/por_empresa/[id]', 'target' => '_blank'),
                                                    );
	...
	{
            if(!empty($valores['ultima_integracao_inicio']))
            {
                $valores['ultima_integracao_inicio'] = converte_data_mysql($valores['ultima_integracao_inicio'].' 00:00');
            }
            if(!empty($valores['ultima_integracao_fim']))
            {
                $valores['ultima_integracao_fim'] = converte_data_mysql($valores['ultima_integracao_fim'].' 23:59');
            }
                $config['itens'] = array(
                                        array( 'name' => 'id',               'titulo' => 'ID: ',                   'tipo' => 'text', 'valor' => '', 'classe' => 'form-control ui-state-default', 'where' => array( 'tipo' => 'where', 	'campo' => 'empresas.id', 	'valor' => '' ) ),
                                        array( 'name' => 'subcategoria',     'titulo' => 'Categoria: ',            'tipo' => 'select', 'valor' => $this->subcategorias_model->get_select(), 'classe' => 'form-control ui-state-default', 'where' => array( 'tipo' => 'where', 	'campo' => 'empresas.id_subcategoria', 		'valor' => '' ) ),
                                        ...
                                        array( 'name' => 'ultima_integracao_inicio','titulo' => 'Ultima integração inicio:','acao' => 'reverte_data_mysql' , 'tipo' => 'text', 'valor' => '', 'classe' => 'form-control ui-state-default data',   'where' => array( 'tipo' => 'where', 	'campo' => 'empresas.ultima_integracao >=', 'valor' => '' ) ),
                                        array( 'name' => 'ultima_integracao_fim',   'titulo' => 'Ultima integração fim:','acao' => 'reverte_data_mysql' , 'tipo' => 'text', 'valor' => '', 'classe' => 'form-control ui-state-default data',   'where' => array( 'tipo' => 'where', 	'campo' => 'empresas.ultima_integracao <=', 'valor' => '' ) ),
